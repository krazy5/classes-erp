...
@section('header', 'Update Payroll Entry')

@section('content')
    @php
        $defaultPayable = class_basename($payroll->payable_type) === 'Teacher'
            ? 'teacher:' . $payroll->payable_id
            : 'user:' . $payroll->payable_id;
        $selectedPayable = old('payable', $defaultPayable);
    @endphp
    <div class='max-w-4xl rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900'>
        <form method='POST' action='{{ route('admin.payrolls.update', $payroll) }}' class='space-y-6'>
            @csrf
            @method('PUT')
            @include('admin.finance.payrolls._form', ['payroll' => $payroll, 'selectedPayable' => $selectedPayable])
            <div class='flex items-center justify-end gap-3'>
                ...
                <button type='submit' class='rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-indigo-700'>Update payroll</button>
            </div>
        </form>
    </div>
@endsection
